...
export const updateStatus = async (
  ...
  try {
    ...
    if (error.message === "TICKET_NOT_ASSIGNED") {
      return res.status(400).json({
        success: false,
        message: "Ticket must be assigned before work can begin",
      });
    }

    if (error.message === "NOT_ASSIGNED_TECHNICIAN") {
    ...
    if (error.message === "NOT_TICKET_OWNER") {
      return res.status(403).json({
        success: false,
        message: "Only the ticket owner can close this ticket",
      });
    }
